import React from 'react';
import { useParams } from 'react-router-dom';

export default function ProjectDetails() {
    const { projectId } = useParams();

    const projectDetails = {
        "e-commerce-application": {
            title: "E-commerce Application",
            description:
                "Built a scalable e-commerce platform using Angular and .NET (C#). Implemented JWT authentication with guards and interceptors, integrated Razorpay for payments, and designed a responsive UI with Bootstrap.",
            technologies: [
                "Angular",
                ".NET (C#)",
                "JWT Authentication",
                "Razorpay",
                "Bootstrap",
            ],
            keyFeatures: [
                "Secure authentication with JWT and role-based access",
                "Payment integration using Razorpay",
                "Dynamic admin dashboard for product management",
                "Optimized for mobile and desktop with responsive design",
                "Microservices for scalability and faster deployments",
            ],
            screenshots: [
                "src/assets/ecommerce-1.png",
                "src/assets/ecommerce-2.png",
            ],
            architecture: "src/assets/ecommerce-architecture.png",
            techStackIcons: [
                "src/assets/angular-logo.png",
                "src/assets/dotnet-logo.png",
                "src/assets/bootstrap-logo.png",
                "src/assets/razorpay-logo.png",
            ],
            link: "https://github.com/yourrepo/e-commerce-application",
        },
        "image-processing-application": {
            title: "Image Processing Application",
            description:
                "Developed a Java-based application for advanced image manipulation, including filtering, color transformations, and histogram generation. The app supports GUI (Swing) and CLI workflows.",
            technologies: [
                "Java",
                "Swing",
                "MVC Architecture",
                "OOP",
                "JUnit Testing",
            ],
            keyFeatures: [
                "Advanced image filtering and transformations",
                "Support for GUI (Swing) and CLI workflows",
                "Histogram generation and analysis",
                "Modular code with MVC architecture",
                "Unit tested with JUnit for high reliability",
            ],
            screenshots: [
                "src/assets/imageprocess-1.png",
                "src/assets/imageprocess-2.png",
            ],
            architecture: "src/assets/imageprocess-architecture.png",
            techStackIcons: [
                "src/assets/java-logo.png",
                "src/assets/swing-logo.png",
                "src/assets/junit-logo.png",
            ],
            link: "https://github.com/yourrepo/image-processing-application",
        },
        "portfolio-website": {
            title: "Portfolio Website",
            description:
                "Created a personal portfolio website using React and Tailwind CSS to showcase skills, projects, and achievements. The site features smooth navigation, responsive design, and interactive elements.",
            technologies: [
                "React",
                "Tailwind CSS",
                "JavaScript",
                "React Router",
            ],
            keyFeatures: [
                "Responsive design for all devices",
                "Dynamic project sections with React Router",
                "Fast and smooth navigation",
                "Interactive elements and modern design",
                "Lightweight and optimized performance",
            ],
            screenshots: [
                "src/assets/portfolio-1.png",
                "src/assets/portfolio-2.png",
            ],
            architecture: "src/assets/portfolio-architecture.png",
            techStackIcons: [
                "src/assets/react-logo.png",
                "src/assets/tailwind-logo.png",
                "src/assets/js-logo.png",
            ],
            link: "https://github.com/yourrepo/portfolio-website",
        }
    };

    const project = projectDetails[projectId];

    if (!project) {
        return <div className="text-center py-20 text-2xl">Project not found.</div>;
    }

    return (
        <div className="p-10 md:p-20 bg-gray-900 text-white min-h-screen">
            <div className="max-w-6xl mx-auto">
                {/* Header Section */}
                <h2 className="text-5xl font-extrabold mb-6 text-Apricot text-center">
                    {project.title}
                </h2>
                <p className="text-lg text-gray-300 leading-relaxed text-center max-w-3xl mx-auto mb-12">
                    {project.description}
                </p>

                {/* Key Features */}
                <div className="bg-gray-800 p-8 rounded-lg shadow-lg mb-16">
                    <h3 className="text-3xl font-semibold mb-6 text-Apricot">Key Features</h3>
                    <ul className="space-y-4">
                        {project.keyFeatures.map((feature, idx) => (
                            <li key={idx} className="flex items-center space-x-4">
                                <span className="h-4 w-4 bg-Apricot rounded-full inline-block"></span>
                                <p className="text-lg">{feature}</p>
                            </li>
                        ))}
                    </ul>
                </div>

                {/* Screenshots */}
                <div className="mb-16">
                    <h3 className="text-3xl font-semibold mb-6 text-Apricot">Screenshots</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        {project.screenshots.map((screenshot, idx) => (
                            <div key={idx} className="relative overflow-hidden rounded-lg shadow-lg hover:scale-105 transition-transform duration-300">
                                <img
                                    src={screenshot}
                                    alt={`Screenshot ${idx + 1}`}
                                    className="w-full"
                                />
                            </div>
                        ))}
                    </div>
                </div>

                {/* Architecture Diagram */}
                <div className="mb-16">
                    <h3 className="text-3xl font-semibold mb-6 text-Apricot">Architecture Diagram</h3>
                    <div className="overflow-hidden rounded-lg shadow-lg hover:scale-105 transition-transform duration-300">
                        <img
                            src={project.architecture}
                            alt="Architecture Diagram"
                            className="w-full"
                        />
                    </div>
                </div>

                {/* Tech Stack */}
                <div className="mb-16 text-center">
                    <h3 className="text-3xl font-semibold mb-8 text-Apricot">Tech Stack</h3>
                    <div className="flex justify-center gap-10 flex-wrap">
                        {project.techStackIcons.map((icon, idx) => (
                            <div key={idx} className="group flex flex-col items-center space-y-2">
                                <img
                                    src={icon}
                                    alt="Tech Stack Icon"
                                    className="h-20 w-20 rounded-lg shadow-md hover:scale-110 transition-transform duration-300"
                                />
                                <span className="text-sm text-gray-400 group-hover:text-Apricot">
                                    {project.technologies[idx]}
                                </span>
                            </div>
                        ))}
                    </div>
                </div>

                {/* GitHub Link */}
                <div className="text-center mt-20">
                    <a
                        href={project.link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="bg-Apricot px-10 py-4 rounded-lg text-lg font-semibold hover:bg-AppleCore hover:text-white transition-all duration-300"
                    >
                        View Project on GitHub
                    </a>
                </div>
            </div>
        </div>
    );
}
